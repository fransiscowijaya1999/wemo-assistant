...
class LookupRepository {
  ...
  /// Search assemblies (diagram/parts groups) by code, name, or the machine
  /// they belong to — same token engine as [search], so "beat cylinder head"
  /// ranks the BeAT cylinder-head assembly by how many words match any field.
  Future<List<AssemblySearchResult>> searchAssemblies(String query) async {
    final q = query.trim();
    if (q.isEmpty) return [];
    ...
    final score = <String, int>{};
    for (final term in terms) {
      for (final id in await _assemblyIdsMatchingTerm(term)) {
        score[id] = (score[id] ?? 0) + 1;
      }
    }
    if (score.isEmpty) return [];
    ...
    final byId = {for (final r in await _assembliesByIds(ids)) r.assemblyId: r};
    return [for (final id in ids) if (byId[id] != null) byId[id]!];
  }

  /// Assembly ids matching one term (substring, case-insensitive) in the
  /// assembly code/name or the owning machine's brand/model.
  Future<Set<String>> _assemblyIdsMatchingTerm(String term) async {
    final pattern = '%$term%';
    final rows = await db.customSelect(
      'SELECT id FROM assemblies WHERE code LIKE ?1 OR name LIKE ?1 '
      'UNION SELECT a.id FROM assemblies a JOIN machines m ON m.id = a.machine_id '
      '  WHERE m.brand LIKE ?1 OR m.model LIKE ?1',
      variables: [Variable<String>(pattern)],
      readsFrom: {db.assemblies, db.machines},
    ).get();
    return {for (final r in rows) r.read<String>('id')};
  }

  /// Display rows for known assembly ids, in the given order.
  Future<List<AssemblySearchResult>> _assembliesByIds(List<String> ids) async {
    if (ids.isEmpty) return [];
    final placeholders = List.filled(ids.length, '?').join(',');
    final rows = await db.customSelect(
      'SELECT a.id, a.code, a.name, a.group_type AS group_type, a.image_ref AS image_ref, '
      "       m.brand || ' ' || m.model AS machine_label, "
      '       (SELECT COUNT(*) FROM assembly_items ai WHERE ai.assembly_id = a.id) AS part_count '
      'FROM assemblies a JOIN machines m ON m.id = a.machine_id '
      'WHERE a.id IN ($placeholders)',
      variables: [for (final id in ids) Variable<String>(id)],
      readsFrom: {db.assemblies, db.machines, db.assemblyItems},
    ).get();
    final byId = <String, AssemblySearchResult>{};
    for (final r in rows) {
      final id = r.read<String>('id');
      byId[id] = AssemblySearchResult(
        assemblyId: id,
        code: r.read<String>('code'),
        name: r.read<String>('name'),
        groupType: r.read<String>('group_type'),
        machineLabel: r.read<String>('machine_label'),
        partCount: r.read<int>('part_count'),
