...
- [x] Substitute highlight (current replacement) — within a substitute cluster the admin marks ONE part as the **current replacement**; the rest read as **obsolete/superseded**. Modeled as a per-part boolean `is_current_replacement` on `parts` (not on the undirected `part_substitutes` pair), so it rides the existing parts sync/backup for free; `getPartFull` already spreads the flag and `getSubstitutes` now returns `isCurrent` per link. New `POST`/`DELETE /parts/:id/substitutes/current` (`requireAdmin`) — marking auto-moves the highlight by clearing the flag on the part's direct substitutes so exactly one per cluster stays current (registered before `/:otherId` so `current` isn't captured as an id). Admin **Substitutes** tab: star toggle on the managed part + a "make current" star on each substitute chip (current styled gold); Browse detail stars the current one, mutes the rest. Mobile clerk **built the substitutes surface from scratch** (the replica previously dropped `partSubstitutes` rows): drift `Parts.isCurrentReplacement` + new `PartSubstitutes` table, `schemaVersion` 1→2 with a destructive drop-recreate migration that resets the cursor for a clean full re-sync; `LookupRepository.partDetail` unions the pair → other side + primary number + note + flag; `PartDetailScreen` gains a status banner ("Current replacement" / "Superseded — obsolete" pointing at the current one) + a Substitutes section (star/obsolete labels, tap-through). Verified: backend curl (mark/auto-move/unmark/sync), admin `tsc`+build+boot, and end-to-end on the Pixel 9 emulator (migration wiped + full re-sync, `Part Substitutes: 1`, both directional banners + tap-through).
- [x] Ingest dot-mapping toggle — "Auto-place dots" switch on the Ingest + Batch views (persisted `wemo.mapDots`, default on). Off threads `mapDots:false` to `POST /ingest/page`, which swaps the extraction system prompt/user text to skip balloon-coordinate estimation (empty `dots`), saving output tokens/thinking on pages where AI dot placement is too rough to keep; the diagram bbox/crop + image upload still run, so the admin places dots by hand in Dot mapping. Backend + admin typecheck + admin build clean.
- [x] Next: correction-assistant `propose_substitute`; backup/restore admin UI buttons. Verified on the admin UI and backend API.
- [x] Bundle code-split: extracted vendor, mantine, icons, and pdf into separate chunks in vite config.
- [x] Multi-brand catalog adapter: Updated AI extraction prompts to dynamically handle Honda, Yamaha, Suzuki, Daihatsu, Toyota, and Mitsubishi manual formats.
- [ ] Future: semantic lookup; web-source adapter.
