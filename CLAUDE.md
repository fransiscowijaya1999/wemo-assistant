...
- [ ] Next: clerk AI assistant (dedicated tab) — read-only + online-only chat grounded in the catalog; needs a clerk-facing backend chat endpoint (Claude seam, read-only catalog tools). Then mobile M4 (visual browse machine→group→grid), M5 polish, serial/variant applicability, color-review UI, semantic lookup, multi-brand/web-source adapter, bundle code-split.
